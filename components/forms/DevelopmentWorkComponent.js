...
import { getDevWorkData } from '../../lib/api';

const DevelopmentWorkComponent = () => {
    ...
    const columns = [
      ...
      React.useEffect(()=>{
        ...
        async function fetchData() {
          const cData = await getDevWorkData() //applo client
    
          // 👇️ only update state if component is mounted
          if (isApiSubscribed) {
            cData.forEach((element,idx) => {
              //console.log('element',element)
              tempData.push({key:idx,sNo:idx+1,title:element.title,date:element.develop.date,download:'Download PDF',slug:element.slug,url:element.develop.downloadUrl
              })
            });
            setSdata(tempData)
    ...
    //   console.log("sData development works :", sData)
  return (
    <div>
        {sData !== null ?
...
export default DevelopmentWorkComponent