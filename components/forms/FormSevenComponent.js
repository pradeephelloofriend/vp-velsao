...
import { getDevWorkData, getFormSevenData } from '../../lib/api';

const FormSevenComponent = () => {
    ...
    const columns = [
      ...
      React.useEffect(()=>{
        ...
        async function fetchData() {
          const cData = await getFormSevenData() //applo client
    
          // 👇️ only update state if component is mounted
          if (isApiSubscribed) {
            cData.forEach((element,idx) => {
              //console.log('element',element)
              tempData.push({key:idx,sNo:idx+1,title:element.title,date:element.form7.date,download:'Download PDF',slug:element.slug,url:element.form7.downloadUrl
              })
            });
            setSdata(tempData)
...
export default FormSevenComponent